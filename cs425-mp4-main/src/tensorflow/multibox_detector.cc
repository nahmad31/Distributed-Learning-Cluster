...
Status ReadLocationsFile(const string& file_name, std::vector<float>* result,
  ...
  while (std::getline(file, line)) {
    ...
    result->push_back(std::stof(line.substr(0, split)));
    result->push_back(std::stof(line.substr(split + 2)));
  }
...
Status ReadTensorFromImageFileMultibox(const string& file_name, const int input_height,
...
// can use to run it.
Status LoadGraphMulitbox(const string& graph_file_name,
                         std::unique_ptr<tensorflow::Session>* session) {
...
// this prints the top five highest-scoring values.
string PrintTopDetections(const std::vector<Tensor>& outputs,
  ...
  Tensor indices;
  Tensor scores;
  Status top_detections_status = GetTopDetections(outputs, how_many_labels, &indices, &scores);
  if (!top_detections_status.ok()) {
    LOG(ERROR) << top_detections_status;
    return "";
  }
  //TF_RETURN_IF_ERROR(
  ...
  //std::cout << "===== Top " << how_many_labels << " Detections ======" << std::endl;
  string top_detections;
  for (int pos = 0; pos < how_many_labels; ++pos) {
    ...
    top_detections += ("Detection " + std::to_string(pos) + ": " + 
                      "L:" + std::to_string(left) + " " +
                      "T:" + std::to_string(top) + " " + 
                      "R:" + std::to_string(right) + " " +
                      "B:" + std::to_string(bottom) + " " +
                      "(" + std::to_string(label_index) + ") score: " + 
                      std::to_string(DecodeScore(score)) + "\n");

    DrawBox(image_width, image_height, left, top, right, bottom, &image_flat);
  }

  if (!image_file_name.empty()) {
    Status save_image_status = SaveImage(*original_tensor, image_file_name);
    if (!save_image_status.ok()) {
      LOG(ERROR) << save_image_status;
      return "";
    }
  }
...
string multiboxDetection(const string& image, std::unique_ptr<tensorflow::Session>& session, const string& box_priors, const string& image_out) {
  ...
  //Status load_graph_status = LoadGraphMulitbox(graph_path, &session);
  //if (!load_graph_status.ok()) {
  ...
  Status read_tensor_status =
      ReadTensorFromImageFileMultibox(image_path, input_height, input_width, input_mean,
  ...
  string top_detections = PrintTopDetections(outputs, box_priors, num_boxes,
                                           num_detections, image_out,
                                           &image_tensors[1]);

  if (top_detections.empty()) {
    LOG(ERROR) << "Getting Top Detections failed";
    return "";
  }
  return top_detections;
}
