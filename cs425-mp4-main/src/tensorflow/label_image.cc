...
Status ReadLabelsFile(const string& file_name, std::vector<string>* result,
  ...
  if (!file) {
    std::cout << "Labels file " << file_name << " not found." << std::endl;
    return Status::OK();
 }
  result->clear();
  string line;
...
static Status ReadEntireFile(tensorflow::Env* env, const string& filename,
  ...
  if (data.size() != file_size) {
    std::cout << "Truncated read of " << filename << " expected " << file_size << " got " << data.size() << std::endl;
    return Status::OK();
  }
...
Status ReadTensorFromImageFileLabel(const string& file_name, const int input_height,
...
// can use to run it.
Status LoadGraphLabel(const string& graph_file_name,
                      std::unique_ptr<tensorflow::Session>* session) {
...
string PrintTopLabels(const std::vector<Tensor>& outputs,
  ...
  Tensor indices;
  Tensor scores;
  Status get_top_labels_status = GetTopLabels(outputs, how_many_labels, &indices, &scores);
  if (!get_top_labels_status.ok()) {
    LOG(ERROR) << get_top_labels_status;
    return "";
  }
  ...
  for (int pos = 0; pos < how_many_labels; ++pos) {
    ...
    top_labels += (labels[label_index] + " (" + std::to_string(label_index) + "): " + std::to_string(score) + "\n");
  }
  return top_labels;
}
...
string labelImage(const string& image, std::unique_ptr<tensorflow::Session>& session, const string& labels) {
  ...
  //Status load_graph_status = LoadGraphLabel(graph_path, &session);
  //if (!load_graph_status.ok()) {
  ...
  Status read_tensor_status =
      ReadTensorFromImageFileLabel(image_path, input_height, input_width, input_mean,
  ...
  string top_labels = PrintTopLabels(outputs, labels);
  if (top_labels.empty()) {
    LOG(ERROR) << "Getting Top Lables failed";
    return "";
  }
